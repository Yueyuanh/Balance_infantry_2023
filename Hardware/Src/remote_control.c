...
/**
  * @brief      图传遥控链路协议解析
  * @param[in]  transfer_image_buf：原生数据指针
  * @param[out] rc_ctrl：遥控器数据指针
  * @retval     none
  * @attention  原生数据包含了裁判系统数据的帧头、命令码id、帧尾部分
  */
...
/**
  * @brief      图传链路串口中断
  * @attention  每次用cubemx重新生成工程后，USART6_IRQHandler函数会在stm32f4xx_it.c和.h中重定义，目前没有其它解决方法，需要手动删除
  */
